...
export function WaitingApprovalsScreen(): React.ReactElement {
  ...
  const handleRowClick = useCallback(
    (approvalRequestId: number) => {
      router.push(`/(tabs)/sales/quotations/${approvalRequestId}`);
    },
    [router]
  );
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={[styles.emptyText, { color: colors.textMuted }]}>
          {t("quotation.noWaitingApprovals")}
        </Text>
      </View>
    );
  }, [isLoading, colors, t]);
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          <Text style={[styles.headerTitle, { color: colors.text }]}>
            {t("quotation.waitingApprovalsList")}
          </Text>
          {data && data.length > 0 && (
  ...
  return (
    <>
      ...
      <View style={[styles.container, { backgroundColor: colors.header }]}>
        <ScreenHeader title={t("quotation.waitingApprovals")} showBackButton />

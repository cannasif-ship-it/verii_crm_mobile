...
export interface LineGroup {
  key: string;
  main: DemandLineDetailGetDto;
  related: DemandLineDetailGetDto[];
}

export function groupDemandLines(
  lines: DemandLineDetailGetDto[]
): LineGroup[] {
  const map = new Map<string, DemandLineDetailGetDto[]>();
...
function mapDetailLineToFormState(d: DemandLineDetailGetDto): DemandLineFormState {
  const base: DemandLineFormState = {
    id: `line-${d.id}`,
...
export function mapDetailLinesToFormState(
  ...
  const groups: DemandLineFormState[][] = [];
...
export function mapDetailRatesToFormState(
  rates: DemandExchangeRateDetailGetDto[],
...
export function mapDemandLineFormStateToCreateDto(
...
export function mapDemandLineFormStateToUpdateDto(
  line: DemandLineFormState,
  demandId: number
): DemandLineUpdateDto | null {

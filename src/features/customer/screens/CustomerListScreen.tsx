...
import { SearchInput, CustomerCard } from "../components";
...
export function CustomerListScreen(): React.ReactElement {
  ...
  const renderItem = useCallback(
    ({ item }: { item: CustomerDto }) => {
      if (!item) return null;
      return <CustomerCard customer={item} onPress={() => handleCustomerPress(item)} />;
    },
    [handleCustomerPress]
  );
  ...
  const keyExtractor = useCallback((item: CustomerDto, index: number) => String(item?.id ?? index), []);
        ...
        <View style={[styles.content, { backgroundColor: contentBackground }]}>
          <View style={styles.topSection}>
            ...
            >
              ...
              <Text style={styles.createButtonText}>{t("customer.create")}</Text>
            </TouchableOpacity>
            <SearchInput
              value={searchText}
